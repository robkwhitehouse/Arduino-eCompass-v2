...
void loop() {
  ...
  runner.execute();
  //Scan the encoder
  if (encoder.active) encoder.scan();   
  
  // Run the foreground task
  if (eventQueue.getCount() > 0 ) {//Something happened
   ...
   Serial.println(event);
    switch ( event ) {
      case LONGPRESS:
        fsm.currentState = menuMode;
        break;
      case SHORTPRESS:
        fsm.currentState = runMode;
        break;
      default:
        Serial.print("Invalid event, value: ");
        Serial.println(event);
...
void runMode()
{
  ...
  if (encoder.active) { //any rotary movement adjusts required heading
    requiredHeading += encoder.getPulseCount();
    if (requiredHeading > 360) requiredHeading -= 360;
    else if (requiredHeading < 0) requiredHeading += 360;
  }
  sprintf(buff,"%03d", requiredHeading);
  display.println(buff);
